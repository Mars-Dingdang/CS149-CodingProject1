...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  typedef __cs149_vec_float vflt;
  typedef __cs149_vec_int vint;
  typedef __cs149_mask vmsk;


  const int B = VECTOR_WIDTH;
  vflt x, result;
  vint y, count;
  vflt clamp = _cs149_vset_float(9.999999f);
  vint zero = _cs149_vset_int(0);
  vint ones = _cs149_vset_int(1);
  vmsk MaskAll, MaskYisZero, MaskYisNotZero, MaskCountIsGreaterThanZero, MaskResultIsGreaterThanClamp;

  MaskAll = _cs149_init_ones();

  // A 0 in the mask means the corresponding lane is inactive, and the instruction will not update the output for that lane
  for (int i = 0; i + B <= N; i += B) {
    _cs149_vload_float(x, values + i, MaskAll);
    _cs149_vload_int(y, exponents + i, MaskAll);
    _cs149_vgt_int(MaskYisNotZero, y, zero, MaskAll);
    _cs149_vset_float(result, 1.f, MaskAll);
    _cs149_vsub_int(count, y, zero, MaskYisNotZero); // count = y - 1;
    while (_cs149_cntbits(MaskYisNotZero) > 0) {
      _cs149_vmult_float(result, result, x, MaskYisNotZero); // result *= x;
      _cs149_vsub_int(count, count, ones, MaskYisNotZero); // count --
      _cs149_vgt_int(MaskCountIsGreaterThanZero, count, zero, MaskYisNotZero);
      MaskYisNotZero = _cs149_mask_and(MaskYisNotZero, MaskCountIsGreaterThanZero);
      _cs149_vgt_float(MaskResultIsGreaterThanClamp, result, clamp, MaskAll); // if (result > clamp) 
      _cs149_vmove_float(result, clamp, MaskResultIsGreaterThanClamp); //  result = clamp;
    }
    _cs149_vstore_float(output + i, result, MaskAll);
  }
  
  for(int i = N - (N % B); i < N; i++) {
    float x = values[i];
    int y = exponents[i];
    if (y == 0) {
      output[i] = 1.f;
